...
fn solve_challenge(fixture_path: &str) -> String {
    ...
    for rucksack in rucksacks {
        let delimiter = rucksack.len()/2;
        let compartment_1 = &rucksack[..delimiter];
        let compartment_2 = &rucksack[delimiter..];

        let overlaps = compartment_1
            .chars()
            .filter_map(|char| compartment_2.find(char).map(|_| *priorities.get(&char).unwrap()))
            .collect::<Vec<i32>>();

        sum += overlaps.first().unwrap()
    }
